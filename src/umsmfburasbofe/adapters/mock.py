...
class MockAdapter(AgentAdapter):
    ...
    def run(self, request: AgentRequest) -> AgentResponse:
        ...
        if role == "product-analyst":
            ...
            target = metadata.get("fixture_target", "umsmfburasbofe_fixture.txt")
            data = {
            ...
            target = metadata.get("task", {}).get("allowed_paths", ["umsmfburasbofe_fixture.txt"])[0]
            target_path = request.cwd / target
            target_path.parent.mkdir(parents=True, exist_ok=True)
            target_path.write_text("UMSMFBURASBOFE deterministic fixture completed\n", encoding="utf-8")
            data = {
                "status": "implemented",
