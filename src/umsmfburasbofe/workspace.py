...
class WorkspaceMirror:
    ...
    def create(self) -> Path:
        ...
        self._git(["init", "-b", "umsmfburasbofe-internal"])
        self._git(["config", "user.name", "UMSMFBURASBOFE Controller"])
        self._git(["config", "user.email", "umsmfburasbofe@local.invalid"])
        self._git(["add", "-A"])
        self._git(["commit", "-m", "UMSMFBURASBOFE isolated baseline"], hooks=False)
        self.baseline_commit = self.head()

        hook = self.workspace / ".git" / "hooks" / "pre-commit"
        hook.write_text(
            "#!/bin/sh\n"
            "echo 'Agent commits are forbidden. The UMSMFBURASBOFE controller owns checkpoints.' >&2\n"
            "exit 73\n",
            encoding="utf-8",
        )
        hook.chmod(0o755)
