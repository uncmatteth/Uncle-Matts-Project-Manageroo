    ...
    *,
    ...
    lines = [
        ...
        *_bullets([summary], "Describe what this project is for."),
        ...
        "## What Has Shipped",
        "",
        *_bullets(shipped_items, "Nothing shipped through UMSMFBURASBOFE yet."),
        "",
        "## What Must Not Break",
        "",
        ...
        *_bullets(proof_items, "Add the commands or manual checks that prove the current state."),
        ...
        "## Operator Notes",
        "",
        *_bullets(note_items, f"Created by UMSMFBURASBOFE on {utc_now()}."),
        "",
    ]
    return "\n".join(lines)
...
def read_project_memory(repo: Path) -> dict[str, Any]:
    path = project_memory_path(repo)
    if not path.exists():
        return {"ok": False, "path": str(path), "content": "", "next_command": "umsmfburasbofe memory init"}
