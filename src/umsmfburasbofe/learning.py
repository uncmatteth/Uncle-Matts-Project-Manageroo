...
def _card_id(card: dict[str, Any]) -> str:
    return f"{card['category']}-{_fingerprint(card['title'], card['destination'], card['recommendation'])}"
...
def generate_learning_cards(
    *,
    ...
    if status == "COMPLETE":
        ...
        summary = str(result.get("product_summary") or "UMSMFBURASBOFE run completed.")
        cards.append(
            _base_card(
                run_id=run_id,
                ...
                recommendation=(
                    "Add a short operator note so future agents know this run completed "
                ...
                payload={
                    "notes": [
                        (
                            f"Learning note: {summary} Run {run_id} completed. "
                            f"Evidence: {run_root or 'run artifacts'}."
                        )
                    ],
                    "proof": [f"UMSMFBURASBOFE run {run_id} completed."],
                },
            )
        )
    else:
        cards.append(
            _base_card(
                ...
                recommendation=(
                    "Keep this blocker as a pending learning item until the operator or agent "
                    "turns it into a concrete fix, check, or skill."
                ),
                evidence=[
        ...
        if failed:
            cards.append(
                _base_card(
                    ...
                    destination="config-or-installer",
                    recommendation=(
                        "Do not freehand this repair. Fix the configured AUTOREVIEW or Clawpatch command "
                        "and rerun the lane."
                    ),
    ...
    if isinstance(content_kinds, dict) and int(content_kinds.get("media", 0) or 0) > 0:
        cards.append(
            _base_card(
                ...
                destination="config-or-installer",
                recommendation=(
                    "This repo contains media. If the requested work depends on visual understanding, "
                    "configure a visual evidence command lane before trusting the run."
                ),
                evidence=[
                    f"media files detected: {content_kinds.get('media')}",
                    f"inventory artifact: {run_root}/artifacts/discovery/inventory.json" if run_root else "",
                ],
                risk="medium",
                priority=60,
                apply_policy="manual_only",
    ...
    if isinstance(content_kinds, dict) and int(content_kinds.get("prose", 0) or 0) > 0:
        cards.append(
            _base_card(
                ...
                destination="config-or-installer",
                recommendation=(
                    "This repo contains prose. If the requested work depends on exact long-document edits, "
                    "use bounded line ranges or configure a document lane."
                ),
                evidence=[
                    f"prose files detected: {content_kinds.get('prose')}",
                    f"inventory artifact: {run_root}/artifacts/discovery/inventory.json" if run_root else "",
                ],
                risk="medium",
                priority=55,
...
def get_learning_card(repo: Path, card_id: str) -> dict[str, Any]:
    ...
    if not path.exists():
        return {
            "ok": False,
            "card_id": card_id,
            "error": "Learning card was not found.",
            "next_command": f"{PUBLIC_COMMAND} learning list",
        }
...
def apply_learning_card(repo: Path, card_id: str, *, approve: bool = False) -> dict[str, Any]:
    ...
    if card.get("apply_kind") != "project_memory_note":
        return {
            "ok": False,
            "card_id": card_id,
            "error": "This learning card is manual-only. No automatic apply is available.",
            "apply_kind": card.get("apply_kind"),
        }
    payload = card.get("payload", {})
    memory_update = ensure_project_memory(
        repo,
...
def format_learning_cards(cards: list[dict[str, Any]]) -> str:
    ...
    for card in cards:
        lines.append(
            f"- {card['id']} [{card.get('risk', 'unknown')} risk, priority {card.get('priority', 0)}] "
            f"{card.get('title', '')}"
        )
...
def format_learning_card(card: dict[str, Any]) -> str:
    lines = [
        f"Learning card: {card['id']}",
        f"Status: {card.get('status', 'unknown')}",
        f"Risk: {card.get('risk', 'unknown')}",
        f"Priority: {card.get('priority', 0)}",
        f"Destination: {card.get('destination', 'unknown')}",
        "",
        card.get("title", ""),
        "",
