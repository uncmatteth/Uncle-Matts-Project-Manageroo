...
def _install_patterns() -> tuple[list[str], list[str], int, int]:
    lead_a = [
        "E5", "G5", "A5", "B5", "A5", "G5", "E5", "D5", "E5", "G5", "B5", "D6", "B5", "A5", "G5", "E5",
        "F#5", "A5", "B5", "C#6", "B5", "A5", "F#5", "E5", "G5", "B5", "D6", "E6", "D6", "B5", "A5", "G5",
    ]
    lead_b = [
        "B4", "E5", "G5", "B5", "A5", "-", "G5", "E5", "D5", "E5", "G5", "A5", "B5", "A5", "G5", "-",
        "C5", "E5", "A5", "C6", "B5", "A5", "E5", "C5", "D5", "F#5", "A5", "D6", "C6", "A5", "F#5", "-",
    ]
    lead_c = [
        "E5", "-", "E5", "G5", "B5", "D6", "B5", "G5", "A5", "-", "A5", "C6", "E6", "C6", "A5", "E5",
        "G5", "B5", "D6", "G6", "F#6", "D6", "B5", "-", "A5", "C6", "E6", "A6", "G6", "E6", "B5", "E6",
    ]
    lead_d = [
        "E6", "D6", "B5", "A5", "G5", "-", "A5", "B5", "D6", "B5", "A5", "G5", "E5", "G5", "A5", "-",
        "F#5", "A5", "C#6", "E6", "C#6", "A5", "F#5", "E5", "G5", "B5", "D6", "F#6", "E6", "D6", "B5", "-",
    ]
...
def _patterns(cue: str) -> tuple[list[str], list[str], int, int]:
    if cue == "success":
        melody = [
            ...
            "G5", "B5", "D6", "G6", "F#6", "D6", "B5", "D6", "A5", "C6", "E6", "A6", "G6", "E6", "B5", "E6",
        ]
        return melody, ["E2"] * 8 + ["F#2"] * 8 + ["G2"] * 8 + ["A2"] * 4 + ["B2"] * 4, 150, 1
    if cue == "build":
        melody = [
            "E5", "-", "E5", "G5", "A5", "-", "B5", "A5", "G5", "E5", "D5", "E5", "G5", "-", "A5", "B5",
            "C6", "B5", "A5", "G5", "E5", "G5", "A5", "-", "B5", "D6", "C6", "B5", "A5", "G5", "E5", "-",
        ]
        return melody, ["E2"] * 8 + ["C2"] * 8 + ["G2"] * 8 + ["D2"] * 8, 156, 3
...
def generate_theme(path: Path, *, cue: str = "install", variant: int = 0) -> Path:
    ...
    for step_index in range(total_steps):
        ...
        for step_sample in range(samples_per_step):
            ...
            frames.append(int(max(-1.0, min(1.0, mixed * fade * MASTER_VOLUME)) * 32767))
...
def _player_command(path: Path) -> list[str] | None:
    ...
    for executable, args in (("ffplay", ["-nodisp", "-autoexit", "-loglevel", "quiet"]), ("mpv", ["--no-video", "--really-quiet"]), ("paplay", []), ("aplay", ["-q"])):
    ...
    if os.name == "nt":
        ...
        if powershell:
            ...
            return [powershell, "-NoProfile", "-Command", f"$p=New-Object System.Media.SoundPlayer '{escaped}'; $p.PlaySync()"]
    return None
