...
1. `.manageroo/PROJECT-MEMORY.md` for durable project identity, shipped
...
`manageroo compact audit --summary SUMMARY.md` before treating the summary
as authoritative.
    ...
    *,
        ...
        atomic_write_text(
            ...
            "\n".join(
                [
                    ...
                    description,
                    "",
                    "Created with MANAGEROO Solo Operator Mode.",
                    "",
                ]
            ),
        )
    ...
    result = runner.run(
        [
            "git",
            "-c",
            "user.name=MANAGEROO Controller",
            "-c",
            "user.email=manageroo@local.invalid",
            "commit",
            "-m",
...
def initialize_project(
    ...
    *,
    ...
    skill_destination = (
        repo / ".agents" / "skills" / "uncle-matts-project-manageroo" / "SKILL.md"
    )
    skill_destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(
        asset_path("skills/uncle-matts-project-manageroo/SKILL.md"),
        skill_destination,
    )
