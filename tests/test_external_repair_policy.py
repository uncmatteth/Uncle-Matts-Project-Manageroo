...
class ExternalRepairPolicyTests(unittest.TestCase):
    ...
    def test_checkpoint_manifest_without_resume_identity_is_rejected(self):
        for missing_field, expected_error in (
            ("input_fingerprint", "inputs have changed"),
            ("lane_index", "lane order has changed"),
        ):
            with self.subTest(missing_field=missing_field), tempfile.TemporaryDirectory() as temp:
                ...
                with self.assertRaisesRegex(SafetyError, expected_error):
                    run_external_review_repair_lanes(
                        fake,
                        brief="repair",
                        plan={"tasks": [{"allowed_paths": ["tracked.txt"]}]},
