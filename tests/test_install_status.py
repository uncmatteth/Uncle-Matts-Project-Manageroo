...
class InstallStatusTests(unittest.TestCase):
    ...
    def test_live_probe_overrides_stale_cached_installed_status(self):
        with tempfile.TemporaryDirectory() as temp:
                ...
                json.dumps(
                    {
                        "launcher": "/tmp/bin/manageroo",
                        "external_tools": [
                            {"name": "codex", "installed": True, "configured": True}
                        ],
                        "stack_summary": {
                            "items": [
                                {
            ...
            with patch("manageroo.install_status.shutil.which", return_value=None), patch(
                "manageroo.install_status._find_skill", return_value=None
            ):
                report = stack_status(lock)
    ...
    def test_uninstall_plan_uses_recorded_custom_launcher_not_default_bin(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            custom_launcher.write_text("custom\n", encoding="utf-8")
