...
def _hold_artifact_lock_with_delayed_owner(
    ...
    lock_path = Path(root) / ".artifact-ledger.lock"
    real_write_text = Path.write_text

    def delayed_owner_write(path: Path, text: str, *args, **kwargs):
    ...
    with mock.patch.object(Path, "write_text", new=delayed_owner_write):
