...
| A capability entrypoint or support file is replaced by a symlink or changes while its instructions and tree digest are ingested | Pin capability roots and directories with descriptor-relative no-follow opens; require stable regular-file identity, size, and metadata before and after each read; derive entrypoint instructions and all hashes from the same descriptor snapshot | Preventive, fails closed |
