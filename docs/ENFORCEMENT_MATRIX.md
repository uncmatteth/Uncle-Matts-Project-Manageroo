...
| Release-sweep repeatedly repairs a finding that cannot pass because the repository baseline is already red | Run configured Manageroo gates before map/review/fix and report exact failing gate output | Preventive; no finding repair starts from an invalid baseline |
| Retryable release-sweep failure silently advances or loses prior repair evidence | Verified stash, exact same-finding reopen with prior failure evidence, continuous attempt count, retained checkpoint, and no final closure before success | Preventive and recoverable in controller |
