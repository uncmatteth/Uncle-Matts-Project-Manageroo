...
| Retryable release-sweep failure silently advances to another finding or restarts without its prior evidence | Verified stash, stash/path feedback recorded through Clawpatch even while the finding remains open, exact same-finding `next`, visibly numbered three-attempt recovery cycles, retained checkpoint, and no final closure before success | Preventive and recoverable in controller |
...
| Relaunched release sweep loses its interrupted current finding | Durable repository, branch, HEAD, finding, phase, cycle, and retry progress reconciled against existing `.clawpatch` state | Recoverable in controller; relaunch remains external |
