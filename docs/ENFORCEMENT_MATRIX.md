...
| Decision artifacts redirect reads, locks, or writes through a nested planning-directory symlink | Reject symlink/reparse-point planning components and require the resolved planning directory to remain under the validated run root before artifact access | Preventive |
