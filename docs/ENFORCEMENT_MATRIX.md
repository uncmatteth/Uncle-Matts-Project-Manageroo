...
| A skill support file changes after import approval while `SKILL.md` stays unchanged, or a source tree contains an unsupported filesystem entry | Require a regular-file `SKILL.md`; reject source entries that are neither regular files nor directories; bind every candidate to a race-checked full-tree digest covering relative paths, file modes, and content before staging | Preventive |
