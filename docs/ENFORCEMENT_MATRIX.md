...
| Windows decodes UTF-8 ClawPatch or provider output with a legacy code page and loses the command result | Decode every supervised child stream explicitly as UTF-8 with replacement for malformed bytes | Preventive, cross-platform output contract |
...
| A same-finding repair keeps producing different partial edits indefinitely | Permit at most three ClawPatch `fix` attempts in one unattended sweep; preserve the combined source and checkpoint at the original HEAD, then stop without advancing, triaging, closing, committing, or pushing | Preventive, bounded stop |
| A stopped ClawPatch process leaves an already-applied attempt dirty | Resume only when checkpoint branch, base HEAD, exact dirty paths, finding state, and one applied or timeout-stranded planned patch record agree; run gates and revalidation first, but convert a gate failure caused by the preserved partial repair into an `open` same-finding continuation and enter the next bounded fix attempt instead of advancing or stopping the queue | Preventive, exact-ownership recovery |
