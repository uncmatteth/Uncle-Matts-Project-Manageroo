...
| Retryable release-sweep failure silently advances to another finding or restarts without its prior evidence | Verified stash, stash/path feedback recorded through Clawpatch even while the finding remains open, exact same-finding `next`, one continuous visibly numbered attempt sequence, retained checkpoint, and no final closure before success | Preventive and recoverable in controller |
