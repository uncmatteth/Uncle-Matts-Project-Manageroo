...
| Revalidation keeps a ClawPatch finding open but the supervisor stops instead of continuing it | Commit only the applied patch-attempt paths as a continuation checkpoint, then use ClawPatch's `next` to select the same open finding for another `show` and `fix` cycle without an arbitrary cap | Preventive, command-owned continuation |
