...
17. `release-ready` is a final operator gate, not a deployment tool. It checks readiness, gates, clean Git state, target, rollback notes, and approval, then writes `.umsmfburasbofe/cache/production-handoff.md`; it does not push, deploy, monitor, or roll back production.
