...
That artifact contains ranked evidence, contradictions, provider errors when applicable, and the authority rule used by the controller. Persisted contradictions must reference at least two distinct artifact items and select one of those items as the preferred evidence. Its discovery identity binds it to the product brief, repository inventory, external-intelligence payload, and integration configuration. Manageroo reuses it only while that identity matches; changed repository or provider state regenerates both the persisted evidence and planning context.
