...
That artifact contains ranked evidence, contradictions, provider errors when applicable, and the authority rule used by the controller. Persisted contradictions must reference at least two distinct artifact items and select one of those items as the preferred evidence. Its discovery identity records the product brief, repository inventory, external-intelligence payload, and integration configuration for correlation only. Manageroo regenerates and validates the artifact on every discovery pass before exposing it to planning, regardless of installer order. Native run evidence is limited to intake and discovery inputs, excluding the previous evidence artifact, so mutable persisted content and later controller outputs are never reused as planning evidence. That location restriction is applied during traversal, before ranking and truncation, so output artifacts cannot crowd eligible planning inputs out of the bundle. Provider result limits count only validated evidence; run-artifact traversal retains a separate bounded raw-read budget and deterministically samples artifact names so one ordered prefix cannot consume it. The planning copy omits the volatile retrieval timestamp while the persisted artifact retains it, keeping equivalent regenerated evidence stable across durable worker retries.

Native project-memory and run-artifact items use bounded excerpts containing a query-term match. Their metadata records the excerpt's character offset and whether the source was truncated; an item is omitted when no matching bounded excerpt can be returned.
