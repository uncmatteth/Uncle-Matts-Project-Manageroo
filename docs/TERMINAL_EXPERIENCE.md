...
When color and animation are available, the banner keeps the name tag alive during install: MANAGEROO gets a scanner effect, the product name gets a color wave, the italic rainbow line says "Offload your thinking so you can really do some thinking...", an ASCII turtle walks left-to-right, and `bttlabs.fun` is emitted as a clickable terminal link when the terminal supports it.
